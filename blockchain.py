...
class Blockchain():
    ...
    def new_data(self, sender, recipient, amount, currency):
        data = {
            'sender' : sender,
            'recipient' : recipient,
            'amount' : amount,
    ...
    def proof_of_work(self, prev_nonce, block):
        
        nonce_valid = False
        nonce = 1

        while nonce_valid is False:     
            nonce_hash = hash_operation(nonce, prev_nonce)

            if nonce_hash.startswith('0'): #easier for sake of research
                nonce_valid = True
            else:
                nonce = nonce + 1
        return nonce
